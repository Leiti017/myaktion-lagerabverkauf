...
# ----------------------------
# MyAktion Lagerabverkauf – QUICK SCAN (nur Foto -> Preise)
# ----------------------------
@app.post("/api/scan")
async def scan_price(file: UploadFile = File(...)):
    """
    Einfache Preisabfrage:
    - Foto aufnehmen
    - Ausgabe: Listenpreis + Unser Preis (-20%)
    """
    ...
    out = RAW_DIR / f"scan_{ts}.jpg"
    out.parent.mkdir(parents=True, exist_ok=True)
    img.save(out, "JPEG", quality=78)

    try:
        import ki_engine_price as price_engine
    except Exception as e:
        return JSONResponse({"ok": False, "error": f"price_engine_import_error: {e}"}, status_code=500)

    start = time.time()
    list_price = price_engine.estimate_list_price_eur(str(out))
    runtime_ms = int((time.time() - start) * 1000)

    if list_price is None:
        return JSONResponse({"ok": False, "error": "ki_failed", "runtime_ms": runtime_ms}, status_code=502)
    ...
    our_price = round(lp * 0.80, 2) if lp > 0 else 0.0
